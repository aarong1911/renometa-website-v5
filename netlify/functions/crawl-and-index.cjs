// netlify/functions/crawl-and-index.js
...
async function crawlAndIndex({ user_request_id, company_site, company_name, email }) {
  try {
    ...
    console.log(`🕷️ Starting Firecrawl for ${company_site}`);
    ...
    const crawlResponse = await fetch('https://api.firecrawl.dev/v1/crawl', {
      ...
      body: JSON.stringify({
        url: company_site,
        limit: 20, // Max pages to crawl (free tier: 500/month)
        scrapeOptions: {
    ...
    // Step 2 — Poll for crawl completion (max 8 minutes)
    let pages = [];
    const maxWaitMs = 480000; // 8 minutes
    ...
    // Step 4 — Generate embeddings and store in Supabase
    ...
    console.log(`✅ Crawl and index complete for ${user_request_id}. ${chunks.length} chunks stored.`);
